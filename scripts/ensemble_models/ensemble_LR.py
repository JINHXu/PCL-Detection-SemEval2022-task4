# ensemble LR: avg_probs(LR(feat1), LR(feat2))
# Diana, Jinghua
...
if __name__ == '__main__':
    ...
    lr_word = LogisticRegression(
        C=1, class_weight='balanced', max_iter=2000, multi_class='ovr', solver='sag')
    lr_word.fit(word_ngrams_train, y_train)

    word_preds = lr_word.predict(word_ngrams_test)
    word_probs = lr_word.predict_proba(word_ngrams_test)

    # char n-gram
    lr_char = LogisticRegression(
        C=1, class_weight='balanced', max_iter=2000, multi_class='ovr', solver='sag')
    lr_char.fit(char_ngrams_train, y_train)

    char_preds = lr_char.predict(char_ngrams_test)
    char_probs = lr_char.predict_proba(char_ngrams_test)

    ensemble_probs = (word_probs+char_probs)/2
    ensemble_preds = np.argmax(ensemble_probs, axis=1)

    preds_path = './predictions/ensemble_LR/task1.txt'
    labels2file([[k] for k in ensemble_preds], preds_path)